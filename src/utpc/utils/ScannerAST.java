 ...
 */
import utpc.utils.Token;
import utpc.utils.TokenAST;

import java.util.ArrayList;

public class ScannerAST {
    ArrayList<TokenAST> tokens;

    public boolean hasMoreToken(){
        return tokens.size()!=1; //剩下最后的";"
    }

    public TokenAST getCurrentToken(){
        return tokens.get(0);
    }
    ...
    private void SplitIntoTokens(String s){
        ...
        for(int i=0; i<s.length(); i++){
            if(split.contains(s.charAt(i)+"")){
                tokens.add(new TokenAST(s.charAt(i)+"", s.charAt(i)+""));
            }
            else{
                ...
                tokens.add(new TokenAST(buffer.toString(), "Variable"));
                i--;
            }
        }
    }

    public ScannerAST(String s){
        tokens = new ArrayList<TokenAST>();
        SplitIntoTokens(s);
    }
